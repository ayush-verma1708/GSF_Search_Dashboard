        ...
        <div className={styles.container}>
          <div className={styles.row}>
            <div className={styles.column}>
              ...
              {/* <div className={styles.itemField}>
                <div className={styles.fieldLabel}>FundingInfo</div>

                <select
                  value={this.state.selectedFundingInfo}
                  onChange={this.handleFundingInfoChange}
                >
                  <option value="">Select FundingInfo</option>
                  {this.state.fundingInfos.map((fundingInfo) => (
                    <option key={fundingInfo.ID} value={fundingInfo.Title}>
                      {fundingInfo.Title}
                    </option>
                  ))}
                </select>
...
EmailID : emailId,
  ...
  //       const companyInfo: any[] = await this._sp.web.lists
  //         .getByTitle("CompanyInfo")
  //         .items.select("Title", "Revenue")
  //         .filter(`Title eq '${item.Company}'`)(); // Filter by the company name

  //       // If company info is found, add it to the tile
  //       let revenue = "N/A"; // Default value
  //       if (companyInfo.length > 0) {
  //         revenue = companyInfo[0].Revenue;
  //       }
  ...
  //   <div class="tile-title">${item.Title}</div>
  //   <div class="tile-company">${item.Company}</div>
  //   <div class="tile-revenue">Revenue: ${revenue}</div>
  //   <div class="tile-buttons">
  //     <button class="delete-button" data-id="${item.ID}">Delete</button>
  //     <button class="apply-button" data-id="${item.ID}">Ready To Invest</button>
  //   </div>
  ...
  // GPT written info about company
  // private getShortlistedItems = async () => {
  //   try {
  //     const items: any[] = await this._sp.web.lists
  //       .getByTitle("Shortlisted")
  //       .items.select("ID", "Title", "Company", "username", "EmailID")();

  //     const user = await this._sp.web.currentUser();
  //     const currentUserTitle = user.Title;
  ...
  //     const companyInfoPromises = items.map(async (item) => {
  //       const companyInfo: any[] = await this._sp.web.lists
  //         .getByTitle("CompanyInfo")
  //         .items.select("Title", "Revenue")
  //         .filter(`Title eq '${item.Company}'`)();
  //       const revenue = companyInfo.length > 0 ? companyInfo[0].Revenue : "N/A";
  //       return { item, revenue };
  //     });

  //     const companyInfoResults = await Promise.all(companyInfoPromises);

  //     companyInfoResults.forEach(({ item, revenue }) => {
  //       if (item.username === currentUserTitle) {
  //         html += `
  // <div class="tile">
  //   <div class="tile-title">${item.Title}</div>
  ...
  //         const revenue =
  //           companyInfo.length > 0 ? companyInfo[0].Revenue : "N/A";
  //         return { item, revenue };
  //       } catch (error) {
  //         console.error("Error fetching company info:", error);
  //         return { item, revenue: "N/A" }; // Provide a default value
  //       }
  //     });

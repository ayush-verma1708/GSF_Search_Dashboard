...
import styles from "./ResultsWebpart.module.scss";
import type { IResultsWebpartProps } from "./IResultsWebpartProps";
...
interface IResultsWebpartState {
  countries: any[];
  industries: any[];
  companies: any[];
  fundingInfos: any[];
...
export default class ResultsWebpart extends React.Component<
  IResultsWebpartProps,
  IResultsWebpartState
> {
  private _sp: SPFI;

  constructor(props: IResultsWebpartProps) {
    super(props);
    this.state = {
      countries: [],
      industries: [],
  ...
  public render(): React.ReactElement<IResultsWebpartProps> {
    return (
    ...
    try {
      ...
      if (filteredItems.length > 0) {
        ...
        .tile a {
          ...
          color: #333; /* Preferred color */
        }
        .tile a:hover {
          color: #666; /* Preferred hover color */
        }
      ...
      filteredItems.forEach((item, index) => {
        ...
        const industry = item.Industry ? item.Industry : "N/A";
        const company = item.Company ? item.Company : "N/A";
        const previousFunding = item.PreviousFunding ? item.PreviousFunding : "N/A";
        const companyPageUrl = item.companyPage ? item.companyPage.Url : "#";
        const CompanyWebsiteUrl = item.CompanyWebsite ? item.CompanyWebsite.Url : "#";
          ...
          <div class="tile">
            <div class="tile-content">
              ...
              <div class="button-container">
                <button class="company-page-button" data-url="${companyPageUrl}">Details</button>
                <button class="company-website-button" data-url="${CompanyWebsiteUrl}">Website</button>
              </div>
            </div>
          </div>
        `;
